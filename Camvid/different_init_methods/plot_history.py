...
index =[]
inits = ["RGB","normalize","std"]
for model_name in model_names:
  for imagescale in imagescales:
    for my_init in inits:
      
      name = my_init+imagescale+model_name + "_" + loss_name
...
for my_index in range(4):
  ...
  ticks =["%d"%i for i in range(0,60,15)]
  ...
  plt.savefig("./summary/init_%s.png"%title_names[my_index],dpi=560)
  # plt.show()
