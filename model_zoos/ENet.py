...
def decoderbottleneck(encoder, output, upsample=False, reverse_module=False):
  internal = output // 4
...
def decoder(m_input, out_dims):
  enet = decoderbottleneck(m_input, 64, upsample=True, reverse_module=True)  # bottleneck 4.0
  enet = decoderbottleneck(enet, 64)  # bottleneck 4.1
  enet = decoderbottleneck(enet, 64)  # bottleneck 4.2
  enet = decoderbottleneck(enet, 16, upsample=True, reverse_module=True)  # bottleneck 5.0
  enet = decoderbottleneck(enet, 16)  # bottleneck 5.1
  
  enet = Conv2DTranspose(filters=out_dims, kernel_size=(2, 2), strides=(2, 2), padding='same')(enet)
  return enet


name = 'enet_naive_upsampling'
def Enet(out_dims, input_shape,
  ...
  enet = encoder(inp)
  enet = decoder(enet, out_dims=out_dims)
